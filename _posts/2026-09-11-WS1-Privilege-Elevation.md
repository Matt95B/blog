...
<div class="box-note">
<p><strong>Note:</strong> Publisher-based rules remain the easiest to maintain long term, but only work if the vendor consistently signs their binaries, not every vendor does. To confirm, on a reference machine with the app installed, confirm the path and check whether the binary is signed:<br>
...
<p>If the binary isn't consistently signed across updates, or you'd rather pin to an exact known-good build, grab its hash instead:<br>
<code>Get-FileHash -Path "C:\Program Files\AppFolder\App.exe" -Algorithm SHA256</code></p>
