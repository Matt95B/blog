...
- Workspace ONE UEM console on version **26.02** or later.
...
{: .box-note}
**Note:** Publisher-based rules remain the easiest to maintain long term, but only work if the vendor consistently signs their binaries, not every vendor does. To confirm, on a reference machine with the app installed, confirm the path and check whether the binary is signed:
```powershell
Get-AuthenticodeSignature -FilePath "C:\Program Files\AppFolder\App.exe" |
    Select-Object Status, SignerCertificate | Format-List
```
If the binary isn't consistently signed across updates, or you'd rather pin to an exact known-good build, grab its hash instead:
```powershell
Get-FileHash -Path "C:\Program Files (x86)\AppFolder\App.exe" -Algorithm SHA256
```
A hash-based rule pins to that exact file. Every time the vendor ships an update that replaces the executable, you'll need to refresh the hash, a path-based rule avoids this maintenance overhead if you're comfortable elevating anything running from that install location.
